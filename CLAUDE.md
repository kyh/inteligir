...
- **AN OPENED FOLDER'S OWN ORIGIN IS ITS BYO REMOTE, AND A FOLDER ANOTHER
  ...
  `apps/cli/src/server/vault/git-engine.ts` under the repo lock and forgetting
  the old remote's tips, so no status calls the vault synced before a pass
  reached the new one. The url grammar is one,
...
- **CONNECTORS ARE THE DEFAULT AGENT'S OWN MCP CONFIG** (reversing the
  ...
  per agent and name, ended by its five-minute window, its row's removal or
  shutdown (`mcp-sign-ins.ts`). The retired registry is deleted at boot, never
...
- **AN ACCOUNT IS DELETED FROM THE APP, THE PASSWORD ASKED AGAIN, THROUGH
  ...
  does, minus the revoke, and leaves the vault alone. No CLI verb: the password
  is a person's to type. `apps/web/src/worker/device/account.ts`,
  `apps/cli/src/server/cloud/sync-runtime.ts`,
...
- **A PHONE COMMENT IS ITS MARKERS AND ITS ENTRY IN ONE CHANGE SET** (owner
  ...
  (`@repo/notes/comments/comment-key`), every entry signed `user`. A reply and
  a resolve are rows of the store alone (`apps/mobile/src/notes/comment-ops.ts`).
  A stale set is settled per path (`reconcileComment` in
  `apps/mobile/src/notes/outbox-reconcile.ts`): the note as a write is, and the
...
- **A PHONE ASKS A MAC THROUGH A CLAIMABLE DISPATCH INBOX, THE REQUEST LANDS
  ...
  so the phone's "a Mac is listening" counts only Macs that would claim.
  Rejected: the `thread_meta` lane, which
...
- **THE .APP SHIPS ITS OWN GIT, AND RUNS IT WHERE THE MAC HAS NONE.** Every
  ...
  Keychain helper, which dugite-native does not build); any other Mac gets the
  bundled one on the server child's env, which the engine, the ACP adapters and
