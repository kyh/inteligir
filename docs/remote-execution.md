...
> protocol**. Make those two interfaces *location-transparent* (executor
...
### Non-goals (for now)
- Literal parity for machine-bound capabilities (controlling *your* Mac, *your*
...
The mobile app is already a *thin client*: the desktop does 100% of execution
...
- **`@repo/agent-host`** — the Electron-free core (lifecycle, session
  resolution, extension framework, paths). Consumers inject bundles/auth/model.
  *(Shipped — slice 1.)*
...
### Slice 0 — Auth on the dispatch relay *(prerequisite for anything holding secrets)*
...
- New `@repo/agent-host` package, Electron-free:
  ...
  - `agent.ts` — `AgentHost`, the parameterized lifecycle over `PiAgent`;
    everything environment-specific is injected lazily.
- Desktop consumes it: `agent/extension.ts` + `agent/paths.ts` become thin
  bindings; `Agent` subclasses `AgentHost`.
- **Delivers:** the agent core can be constructed and run with injected
  bundles/auth/config — the keystone. No behavior change for desktop.
...
- ⚠️ **Filesystem caveat:** executor code that orchestrates *APIs* relocates
  cleanly; executor code that touches the *local workspace* does not. Pair with
...
- **Recommendation: local-first hybrid.** Local stays source of truth (offline
  works). Treat the cloud env as a *working copy*:
...
| Shell / file tools | ✅ | Against the *container* workspace, not your laptop |
| `peekaboo` (macOS automation) | ❌ | Controls *your* Mac — degrade or route back |
...
2. **Custody shift is explicit.** Today OAuth/secrets live on-device and never
   leave. Hosting the executor makes *us* the custodian of user credentials —
   a deliberate trust change that needs isolation, encryption at rest, and a
   clear data-handling story.
3. **Least privilege per session.** A cloud env gets a *scoped* model credential
   and a *scoped* executor token, not the user's full keychain. Reclaim on
   session end.
