...
// where chrome is drawn. `packages/ui/src/ai` is absent on purpose: it keeps its own sizes until a
// surface draws it.
const CHROME_ROOTS = [
...
const PROSE_SIZES = new Map<string, ProseSize>([
  ...
  [
    ...
    {
      reason:
        "not a type size: the drag gutter takes a heading's size so its em-tall box centres on that heading's first line",
      sizes: ["text-[22px]", "text-[16px]", "text-[15px]"],
    },
  ],
]);
