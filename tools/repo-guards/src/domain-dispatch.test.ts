...
const namesEveryMember = (source: string, members: readonly string[]): boolean =>
  members.every((member) => new RegExp(`["'\`]${member}["'\`]`, "u").test(source));

const totalDispatchSites = (vocabulary: Vocabulary): string[] =>
  workspaces()
