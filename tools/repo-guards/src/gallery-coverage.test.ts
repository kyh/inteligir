...
const NOT_DEMOED = new Map<string, string>([
  ...
  [
    ...
    "Internal to sidebar.tsx — its rows are demoed through the Sidebar composition.",
  ],
]);

const COMPONENT_FILE = /\.tsx$/u;

const componentNames = (dir: string): string[] => {
  ...
  it("every @repo/ui component is demoed, or says why not", () => {
    ...
    for (const root of demoedRoots()) {
      for (const name of componentNames(root.dir)) {
        ...
        if (source.includes(`${UI_PACKAGE}/${root.subpath}/${name}"`)) {
          continue;
        }
