...
/** The whole first paint in one answer: root, listing, and the bytes of the
 * note the shell last had open (resolved by the HOST of these ports, so the
 * session never has to learn which note it wants before it can ask). */
...
/** A rename's verdict, in the host's own words when it refuses. */
...
export function createVaultSession(ports: VaultSessionPorts): VaultSession {
   ...
   * clean. The runtime's pre-flush hook drains the Rich editor's serialize
   * debounce first, so a keystroke still in it lands. */
