...
const openingTags = (text: string): { tag: string; line: number }[] => {
  ...
  while (from !== -1) {
    ...
    tags.push({ line: text.slice(0, from).split("\n").length, tag: text.slice(from, end + 1) });
...
describe("raw buttons in the editor", () => {
  it("carry no native title, so the tooltip is always the product's", () => {
    ...
    for (const file of sources(SRC)) {
      ...
      if (NATIVE_TITLE_ALLOWED.has(name)) {
        continue;
      }
      for (const { tag, line } of openingTags(fs.readFileSync(file, "utf-8"))) {
  ...
  it("every allowance still names a file that carries a native title", () => {
    const stale = [...NATIVE_TITLE_ALLOWED.keys()].filter((name) => {
      ...
      return !openingTags(fs.readFileSync(file, "utf-8")).some(({ tag }) => /\btitle=/u.test(tag));
    });
    expect(
      stale,
