...
// editor, and under a split view "the open note" is plural — so the note it
// must act on is the one ITS editor mounted for. The mounted editor owns both
// registrations; this module is just the map. Keyed by path so a consumer can
// never grab a stale editor serving some other note.

import type { SlateEditor } from "platejs";

const editors = new Map<string, SlateEditor>();
// Weak on purpose: an unmounted editor is garbage the moment its pane drops
// it, and a strong reverse map would pin every editor a session ever built.
...
/** The note `editor` was mounted for — the pane identity a non-React caller
 * has. Null for an editor no pane registered (a headless mount). */
