...
const splitPayload = (value: string): SplitPayload | null => {
...
export const clearCanvasGrid = (value: string): string => {
  const split = splitPayload(value);
  if (split === null) {
    return value;
  }
...
export const strokeSegmentCells = (from: CanvasCell, to: CanvasCell): CanvasCell[] => {
