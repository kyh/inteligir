 ...
 * setup (binary install, config seed, etc.) so each integration can own its
 * full lifecycle in one module.
 *
 * Bundles are auto-discovered by the consumer (e.g. the desktop app globs
 * `agent/<name>/extension.ts` default exports). This package only owns the
 * framework — the types, the parallel setup runner, schema validation, and
 * the factory wrapping — so the same machinery runs whether the bundles are
 * Electron-coupled (desktop) or headless (a cloud runner).
 *
 * setup() and register() receive distinct contexts so each phase only sees
 * what's meaningful at that point in the lifecycle.
 */

import type { ExtensionAPI, ExtensionFactory } from "@repo/pi-driver/pi-types";

/**
 * Progress report for long-running setup steps (downloads, runtime installs),
 * surfaced to whatever onboarding UI the consumer wires up. `percent` is null
...
/** Narrow an unknown to a plain object. Local so this package has no shared-dep. */
...
export type ExtensionRegisterContext = {
  /** Shared bin dir on PATH for installed CLIs (e.g. ~/.inteligir/bin). */
  binDir: string;
};
...
export type ExtensionSetupContext = ExtensionRegisterContext & {
  ...
  /**
   * Report progress to the onboarding UI. Bundles call this around long-running
   * operations (downloads, runtime installs). `percent` is null when the step
   * has no measurable progress.
   */
...
export type PiExtensionBundle = {
   ...
   * the bundle reaching into module-level helpers.
   */
    ...
    bundles
      ...
      .map(async (bundle) => {
        try {
          ...
          console.error(`[agent-host] ${bundle.name} setup failed (continuing):`, err);
        }
      }),
  );
 ...
 * schema validation. Used by AgentHost when constructing the session.
 */
