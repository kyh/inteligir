...
function FilePreviewImage({ file, size }: { file: File; size: number }) {
  ...
  return (
    <div
      style={{ width: size, height: size }}
      className="flex items-center justify-center overflow-hidden rounded-lg border border-border bg-muted p-1 text-center text-[10px] break-all text-muted-foreground"
    >
      {file.name}
    </div>
  );
...
function FilePreviewTile({ file, onRemove, size }: FilePreviewTileProps) {
  ...
  return (
    ...
    >
      <FilePreviewImage file={file} size={size} />
...
const InputMessage = forwardRef<HTMLDivElement, InputMessageProps>(
  (
    ...
    const {
      ...
      // the composer would otherwise treat as submit/history/suggestions.
    ...
    const handleKeyDown = useCallback(
      (e: ReactKeyboardEvent<HTMLTextAreaElement>) => {
        ...
        if (suggestionsOpen && !e.shiftKey && !e.altKey && !e.metaKey && !e.ctrlKey) {
          if (e.key === "ArrowDown") {
            e.preventDefault();
          ...
          if (activeSuggestion != null) {
            ...
            if (e.key === "Enter") {
              ...
              if (active !== undefined) acceptSuggestion(active);
              return;
            }
            if (e.key === "Escape") {
              e.preventDefault();
