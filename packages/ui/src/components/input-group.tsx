...
const InputGroup = ({ className, ...props }: React.ComponentProps<"div">) => (
  <div
    ...
    // oxlint-disable-next-line jsx-a11y/prefer-tag-over-role -- <fieldset> and friends carry form semantics and UA styling this layout wrapper must not
    role="group"
    className={cn(
...
const inputGroupAddonVariants = cva(
  "flex h-auto cursor-text items-center justify-center gap-2 py-1.5 text-subtitle font-medium text-muted-foreground select-none group-data-[disabled=true]/input-group:opacity-50 **:data-[slot=kbd]:rounded-2xl **:data-[slot=kbd]:bg-muted-foreground/10 **:data-[slot=kbd]:px-1.5 [&>svg:not([class*='size-'])]:size-4",
  {
    defaultVariants: {
      align: "inline-start",
    },
...
const InputGroupAddon = ({
  ...
  // oxlint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-noninteractive-element-interactions -- click-to-focus affordance for the wrapped input, which keeps the keyboard path
  <div
    // oxlint-disable-next-line jsx-a11y/prefer-tag-over-role -- <fieldset> and friends carry form semantics and UA styling this layout wrapper must not
    role="group"
