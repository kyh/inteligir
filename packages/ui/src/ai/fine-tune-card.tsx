...
const FineTuneSection = ({
  ...
  ref,
  ...props
}: FineTuneSectionProps & RefAttributes<HTMLDivElement>) => (
  <div
    ref={ref}
...
const ScrubField = ({
  ...
  const onPointerDown = (event: PointerEvent<HTMLSpanElement>) => {
    ...
    drag.current = { value, x: event.clientX };
  };
  ...
  return (
    ...
    >
      <span
        // oxlint-disable-next-line jsx-a11y/prefer-tag-over-role -- a pointer-drag track, not an <input type="range">
        role="slider"
        aria-label={label}
        aria-valuenow={value}
        aria-valuemin={min}
...
const SegmentOption = ({
  ...
  ref,
  ...props
}: SegmentOptionProps & RefAttributes<HTMLButtonElement>) => (
  <button
    ref={ref}
    type="button"
    aria-pressed={active}
...
const FineTuneMenuItem = ({
  className,
  children,
  meta,
  ref,
  ...props
}: FineTuneMenuItemProps & RefAttributes<HTMLButtonElement>) => (
  <button
    ref={ref}
    type="button"
    data-menu-row
