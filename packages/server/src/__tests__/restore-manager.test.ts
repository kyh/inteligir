...
describe("RestoreManager.capture (chat)", () => {
  it("copies the pre-write bytes of an existing file as kind 'edit' and notifies", () => {
    ...
    rig.mgr.capture({ origin: "chat", target: { rel: "notes/a.md", tool: "edit" } });
  ...
  it("records a new-file write as kind 'create' (undo = delete it)", () => {
    const rig = makeRig();
    rig.mgr.capture({ origin: "chat", target: { rel: "fresh.md", tool: "write" } });
  ...
  it("captures nothing for an edit of a missing file (pi's edit will ENOENT — no write to protect)", () => {
    const rig = makeRig();
    rig.mgr.capture({ origin: "chat", target: { rel: "missing.md", tool: "edit" } });
  ...
  it("throws on any non-ENOENT read failure — the gate blocks the write (fail-closed)", () => {
    ...
    expect(() => mgr.capture({ origin: "chat", target: { rel: "a.md", tool: "edit" } })).toThrow(
      "EACCES",
    );
  });

  it("a notifier failure never blocks the write — the capture is already on disk", () => {
    const snapshots = memorySnapshots();
    ...
    expect(() =>
      mgr.capture({ origin: "chat", target: { rel: "a.md", tool: "edit" } }),
    ).not.toThrow();
...
describe("RestoreManager.capture (delegation)", () => {
  it("persists the caller-provided bytes under the delegation id, silently (no toast event)", () => {
    const rig = makeRig({ "a.md": "live bytes that may already differ" });
    rig.mgr.capture({
      origin: "delegation",
      id: "delegation-1",
      path: "a.md",
    ...
    expect(rig.events).toEqual([]); // never leaks into the chat undo toast
    expect(rig.snapshots.read("delegation-1")).toMatchObject({
      ok: true,
      origin: "delegation",
      kind: "edit",
      content: "resolved bytes",
...
describe("RestoreManager.restoreChatEdits", () => {
  it("writes the pre-write bytes back byte-exactly (CRLF, no trailing newline)", async () => {
    ...
    const result = await rig.mgr.restoreChatEdits([id]);
  ...
  it("is a no-op success (no write, no watcher churn) when the file already matches", async () => {
    ...
    expect(await rig.mgr.restoreChatEdits([id])).toEqual({ ok: true });
    expect(rig.writes).toEqual([]);
  });

  it("recreates a since-deleted file from the capture", async () => {
  ...
  it("restores multiple files in one call and aggregates failures without giving up", async () => {
    ...
    rig.mgr.capture({ origin: "chat", target: { rel: "a.md", tool: "edit" } });
    rig.mgr.capture({ origin: "chat", target: { rel: "b.md", tool: "edit" } });
    const [first, second] = rig.events;
    rig.vault.set("a.md", "A rewritten");
    rig.vault.set("b.md", "B rewritten");

    const result = await rig.mgr.restoreChatEdits([
      "unknown-id",
      first?.id ?? "",
      second?.id ?? "",
    ]);
    ...
    // The known captures still restored.
  ...
  it("refuses delegation-origin snapshots — their restore lives behind the guarded dock channel", async () => {
    ...
    const result = await rig.mgr.restoreChatEdits(["delegation-1"]);
    expect(result.ok).toBe(false);
