...
src/
  ...
  threads.ts          # thread rows, the keyset-paged listing, the lifecycle CAS, origin
                      # rebinding on rename (one transaction per rename),
...
moving nothing, `listThreads` walking a thousand threads a page at a time, each
exactly once across millisecond ties and into the archived segment, every page
answered from its partial index with no temp b-tree; FIFO claims across connections and same-millisecond bursts;
