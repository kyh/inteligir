...
Inteligir-specific composition over the pi wrapper (`@repo/agent/pi/*`). Each "extension" plugs a third-party capability (connected APIs via the executor, agent-browser, native macOS automation) into pi as a tool.
...
The mechanics of CLI install (GitHub release fetch, sha256 verify, atomic rename) live in `@repo/installer/install` as a single generic primitive — see the package README. Each bundle just calls it with its own naming convention.
