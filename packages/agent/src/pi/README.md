# `packages/agent/src/pi` — the pi quarantine, and the Harness contract

Paths below are relative to `packages/agent/src/` unless a package is named.
...
`__tests__/pi-quarantine.test.ts` — with exactly three exceptions:
`provider/faux-provider.ts` (pi-ai's stub provider; lives in
`provider/` because it is part of the provider menu, but it is pi-shaped) and
two tests (`__tests__/pi-path-parity.test.ts`,
`provider/__tests__/faux-provider.test.ts`). Everything else — the rest of
`@repo/agent`, the Bridge handlers, delegation, the entire renderer — speaks
only to the wrappers here:
...
is just pi with indirection, and the real shape is dictated by the second
implementation's constraints, which nobody knows yet. This section is the map
...
1. **Model resolution.** The host composes a neutral
   ...
   `__tests__/model-selection.test.ts`). Framework model types must not cross
   out of `pi/` into the neutral `agent.ts` API.
...
2. **Token custody / credential store.** Provider OAuth credentials live
   ...
   CLAUDE.md § Decisions; pi's handle is `ModelRuntime`, whose credential
   store owns the auth.json format). A replacement needs
   the same story: interactive OAuth login, `hasAuth`-style probes
   (`auth.ts` composes them via `pi/auth.ts`'s `readStoredCredential`), logout that
...
3. **`tool_call` interception — THE HARDEST REQUIREMENT.** The fail-closed
   private-note boundary (`privacy/`) hangs off pi's blockable
   ...
   harness's own tools do (`privacy/pi-path-parity.ts` mirrors pi's
...
4. **Streaming events.** The app consumes exactly eight event types
   (`packages/bridge/src/agent-event-parser.ts`): `agent_start`, `agent_end`,
   ...
   (`pi/agent.ts::buildPromptFailureEvents`).

5. **Extension / tool registration.** Capabilities reach the agent as
   extension bundles (`bundles.ts`, static registry + disk-drift test)
...
- **Reimplemented:** everything in `pi/*`, the faux stub, the
  `tool_call` hook wiring in `privacy/extension.ts` (+ a new
  path-parity module; the gate core `privacy/gate.ts` is pure and stays),
  and possibly an event adapter for seam 4.
- **Untouched:** the renderer (Bridge-only, harness-agnostic), `@repo/notes`,
  the Bridge handlers, provider catalog/service/store (neutral selections),
  delegation manager, app-machine — they all drive the `agent.ts`
  `Agent` API, which carries no pi types.
