...
1. Add to `AppEventSchema` in `@repo/core/app-state` (validates the Bridge payload).
...
1. Add to `AppStateSchema` in `@repo/core/app-state`.
...
Domain-grouped under `handlers/` (one file per domain, composed by `handlers/register-handlers.ts`). Each group registers through the typed registrar (`lib/handler-registry.ts::collectHandlers`): the channel, TypeBox payload schema, and result type are looked up from the shared registry (`@repo/core/ipc-registry`), payloads are `Value.Check`-validated before the handler runs, and boot throws if any host-owned method is left unhandled. The registry's `UPDATE_METHODS` trio is deliberately absent — electron-updater is the desktop shell's overlay. Host → UI events use `events.ts::emitEvent`, keyed by the same registry — a renamed channel or changed payload shape is a compile error on both sides, and each shell's bridge is derived from the registry too.
