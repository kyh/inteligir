...
// makes the background sweep converge: its own writes disqualify their notes
// from the next pass. OFF by default; the toggle is the whole control
// surface, the voice model's precedent.
...
export const noteIntelligenceRoutes = {
  status: defineRoute({
    path: "/note-intelligence",
    method: "get",
    request: noRequest(),
    response: jsonResponse<NoteIntelligenceStatus>(),
  }),
  toggle: defineRoute({
    path: "/note-intelligence/toggle",
    method: "post",
    request: jsonRequest<EmptyInput, NoteIntelligenceToggle>(noteIntelligenceToggleSchema),
    response: jsonResponse<NoteIntelligenceStatus>(),
  }),
};
