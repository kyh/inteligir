...
describe("JsonStore write-time validation", () => {
  ...
  it("rejects synchronously on a coalesced store before any disk write is dispatched", async () => {
    const fs = gatedAsyncFs();
    const store = new JsonStore<unknown[]>("/test.json", NumbersSchema, [], {
      fs,
      coalesceWrites: true,
    });
    ...
    await store.flush(); // nothing pending — resolves immediately
    expect(fs.files.has("/test.json")).toBe(false);
  });
...
// Async adapter whose writes only complete when the test releases them —
// lets a test hold a write in flight and observe coalescing behavior.
function gatedAsyncFs(): FsAdapter & {
  files: Map<string, string>;
  writes: { content: string; release: () => void; fail: (err: Error) => void }[];
} {
  const files = new Map<string, string>();
  const writes: { content: string; release: () => void; fail: (err: Error) => void }[] = [];
  return {
    files,
    writes,
    read: (path) => files.get(path) ?? null,
    ...
    writeAsync: (path, content, opts) =>
      new Promise<void>((resolve, reject) => {
        writes.push({
          content,
          release: () => {
            if (opts?.shouldAbort?.()) {
              resolve(); // closed store: adapter aborts without touching disk
              return;
            }
            files.set(path, content);
            resolve();
...
describe("JsonStore coalesced writes", () => {
  it("coalesces a burst of writes into one in-flight + one trailing write", async () => {
    const fs = gatedAsyncFs();
    const store = new JsonStore<number[]>("/test.json", NumbersSchema, [], {
      fs,
      coalesceWrites: true,
    ...
    // One write dispatched so far; the rest coalesce behind it.
    expect(fs.writes).toHaveLength(1);
    fs.writes[0]?.release();
    // Yield until the drain dispatches the single trailing write, then let it land.
    while (fs.writes.length < 2) await Promise.resolve();
    fs.writes[1]?.release();
    await store.flush();

    // Trailing write carries the latest cache, not one write per call.
    expect(fs.writes).toHaveLength(2);
    expect(JSON.parse(fs.files.get("/test.json") ?? "")).toEqual([3]);
    expect(store.read()).toEqual([3]);
  });

  it("flush resolves only after the disk write landed", async () => {
    const fs = gatedAsyncFs();
    const store = new JsonStore<number[]>("/test.json", NumbersSchema, [], {
      fs,
      coalesceWrites: true,
    ...
    fs.writes[0]?.release();
    await flushed;
    expect(JSON.parse(fs.files.get("/test.json") ?? "")).toEqual([7]);
  });

  it("flush rejects when the disk write fails, and a later flush retries", async () => {
    const fs = gatedAsyncFs();
    const store = new JsonStore<number[]>("/test.json", NumbersSchema, [], {
      fs,
      coalesceWrites: true,
    ...
    fs.writes[0]?.fail(new Error("disk full"));
    await expect(failing).rejects.toThrow("disk full");
    expect(fs.files.has("/test.json")).toBe(false);

    // The cache stayed ahead of disk; a retry flush re-drains it.
    const retried = store.flush();
    fs.writes[1]?.release();
    await retried;
    expect(JSON.parse(fs.files.get("/test.json") ?? "")).toEqual([5]);
    expect(consoleError).toHaveBeenCalledWith(
      expect.stringContaining("coalesced write failed"),
      expect.any(Error),
    );
  });

  it("close() aborts a pending coalesced write so nothing resurrects the file", async () => {
    const fs = gatedAsyncFs();
    const store = new JsonStore<number[]>("/test.json", NumbersSchema, [], {
      fs,
      coalesceWrites: true,
    ...
    store.write([9]);
    store.close();
    fs.writes[0]?.release(); // adapter sees shouldAbort() === true and skips the disk touch
    await store.flush();

    expect(fs.files.has("/test.json")).toBe(false);
  });

  it("does not invalidate the cache while a write is pending", async () => {
    const fs = gatedAsyncFs();
    fs.files.set("/test.json", JSON.stringify([1]));
    const store = new JsonStore<number[]>("/test.json", NumbersSchema, [], {
      fs,
    ...
    expect(store.read()).toEqual([1]);
    store.write([2]);
    store.invalidate(); // must be a no-op: disk still holds [1]
    expect(store.read()).toEqual([2]);

    fs.writes[0]?.release();
    await store.flush();
    store.invalidate(); // now safe — disk caught up
    expect(store.read()).toEqual([2]);
  });

  it("falls back to the sync adapter write when writeAsync is missing", async () => {
    const fs = memoryFs();
    const store = new JsonStore<number[]>("/test.json", NumbersSchema, [], {
      fs,
      coalesceWrites: true,
    ...
    store.write([4]);
    await store.flush();
    expect(JSON.parse(fs.files.get("/test.json") ?? "")).toEqual([4]);
  });
});
