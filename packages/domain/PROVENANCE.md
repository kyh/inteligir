...
- **Upstream**: https://github.com/get-bb/bb, directory `packages/domain`
...
Every shipped file here is upstream's, kept under upstream's names so a
re-vendor diffs cleanly. The suites under `src/__tests__/` and the scaffolding
are house, and house files may land beside these later — so the record is per
file rather than whole-directory, and the manifest is what says which is which.
