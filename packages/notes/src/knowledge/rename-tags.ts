...
import type { InlineTagSpan } from "./link-extract";
...
const rewriteInlineTags = (content: string, from: string, to: string): string => {
  const replacements: { span: InlineTagSpan; text: string }[] = [];
...
const rewriteFrontmatterTags = (content: string, from: string, to: string): string => {
  ...
  const properties = parsed.properties.map((prop): TypedProperty => {
    if (prop.key !== "tags" || prop.type !== "tags") {
      return prop;
    }
...
export const renameTagsInDoc = (content: string, from: string, to: string): string =>
  rewriteFrontmatterTags(rewriteInlineTags(content, from, to), from, to);
