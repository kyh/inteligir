// Moss's tab-group container (vendored moss-skills is the contract):
//
//   :::tabs
//   === Label
...
// nodes between them into a `mossTabs` block holding `mossTabPanel` children.
...
type PanelContent = Exclude<RootChild, { type: "yaml" | "mossTabs" | "mossTabPanel" }>;

export interface MossTabPanel extends Node {
  type: "mossTabPanel";
  label: string;
  children: PanelContent[];
}

export interface MossTabs extends Node {
  type: "mossTabs";
  children: MossTabPanel[];
}

declare module "mdast" {
  interface RootContentMap {
    mossTabPanel: MossTabPanel;
    mossTabs: MossTabs;
  }
...
function asPanelContent(node: RootChild): PanelContent | null {
  ...
  if (node.type === "yaml" || node.type === "mossTabs" || node.type === "mossTabPanel") {
    return null;
  }
  return node;
}
 ...
 * mossTabs node. Aborts (leaving bytes as parsed) on: content before the
...
const mossTabsToMarkdown: ToMarkdownExtension = {
  handlers: {
    mossTabs: (node: MossTabs, _parent, state, info) => {
...
export const remarkMossTabs: Plugin = function (this: Processor): Transformer {
  const data = this.data();
  (data.toMarkdownExtensions ??= []).push(mossTabsToMarkdown);
  return (tree) => {
