...
export class RuntimeTurnState {
  ...
  private readonly activeTurnWaitersByThreadId = new Map<string, Set<PendingActiveTurnWaiter>>();

  clear(): void {
    this.activeTurnIdByThreadId.clear();
    // Snapshot first: resolveWaiters deletes entries mid-iteration.
    const threadIds = Array.from(this.activeTurnWaitersByThreadId.keys());
  ...
  waitForActiveTurn(args: WaitForActiveTurnStateArgs): Promise<string | null> {
    ...
    return new Promise((resolve) => {
      const waiters =
        this.activeTurnWaitersByThreadId.get(args.threadId) ?? new Set<PendingActiveTurnWaiter>();
