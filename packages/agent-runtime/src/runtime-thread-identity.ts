...
export class RuntimeThreadIdentityRegistry {
  ...
  createProviderState(args: CreateRuntimeProviderIdentityStateArgs): RuntimeProviderIdentityState {
    return {
  ...
  waitForProviderThreadIdentity(args: WaitForProviderThreadIdentityArgs): Promise<string | null> {
  ...
  resolveProviderEventThreadId(args: ResolveProviderEventThreadIdArgs): string | undefined {
    if (args.sourceThreadId && args.providerState.threadIds.has(args.sourceThreadId)) {
      return args.sourceThreadId;
    }

    if (args.eventThreadId && args.providerState.threadIds.has(args.eventThreadId)) {
    ...
    if (lookupId) {
      for (const [bbThreadId, providerThreadId] of this.threadToProviderThread) {
        if (providerThreadId === lookupId && args.providerState.threadIds.has(bbThreadId)) {
          return bbThreadId;
        }
      }
    }
  ...
  resolvePendingIdentityWaiters(providerState: RuntimeProviderIdentityState): void {
