...
// Patched: upstream keys this union off the generated ServerNotification
// type, whose transitive type closure is most of the generated tree. The
// pruned vendor keeps only the types the adapter's own commands touch, so
// the method union derives from the table below instead — the same set of
// names, now self-authoritative. A method Codex adds shows up as coverage
// "unknown" (a provider/unhandled event) rather than a compile error.
...
function isCodexServerNotificationMethod(method: string): method is CodexServerNotificationMethod {
...
function describeParsedCodexRawEvent(event: CodexRawEvent): ProviderRawEventDescription {
  switch (event.kind) {
