...
// Trimmed: the goal, sub-agent, collab-agent, dynamic-tool and raw-response
// item schemas, and the rate-limit snapshot family, are not carried — their
// consumers (goals, provider subagents, dynamic tools, raw shell-output
// recovery, rate-limit surfacing) were trimmed from the adapter. An event
// whose schema is gone parses as unhandled and surfaces as
// `provider/unhandled` instead of crashing.
...
const codexItemStatusSchema = z.enum(["inProgress", "completed", "failed", "declined"]);
...
const codexToolReferenceStatusSchema = z.enum(["inProgress", "completed", "failed", "declined"]);
...
export type CodexAdditionalPermissions = z.infer<typeof codexAdditionalPermissionsSchema>;

const codexRequestPermissionsSchema = z.object({
  network: codexNetworkPermissionsSchema.nullable(),
  fileSystem: codexFileSystemPermissionsSchema.nullable(),
});
export type CodexRequestedPermissionProfile = z.infer<typeof codexRequestPermissionsSchema>;
