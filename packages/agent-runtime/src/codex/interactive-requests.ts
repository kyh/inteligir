...
// Patched: the plan-review subject branch is gone with the trimmed payload
// union (it is claude-code's ExitPlanMode approval; Codex never raises one —
// upstream's branch only threw).
...
function hasGrantablePermissions(
  ...
  return (
    ...
    (fileSystem !== null && (fileSystem.read.length > 0 || fileSystem.write.length > 0))
  );
}
...
export function decodeCodexInteractiveRequest(
  ...
  switch (request.method) {
    case "item/commandExecution/requestApproval": {
      const parsed = codexCommandExecutionRequestApprovalParamsSchema.safeParse(request.params);
      if (!parsed.success) {
        return null;
      }
      const availableDecisions = parseCodexAvailableDecisions(parsed.data.availableDecisions);
      ...
      return {
        ...
        payload: {
          ...
          availableDecisions: filterSessionDecisionWithoutGrant(availableDecisions, sessionGrant),
        },
      };
    }
    case "item/fileChange/requestApproval": {
      const parsed = codexFileChangeRequestApprovalParamsSchema.safeParse(request.params);
      if (!parsed.success) {
        return null;
      }
      const sessionGrant: PendingInteractionGrantablePermissionProfile | null = parsed.data
        .grantRoot
        ? {
            network: null,
            fileSystem: {
    ...
    case "item/permissions/requestApproval": {
      const parsed = codexPermissionsRequestApprovalParamsSchema.safeParse(request.params);
      if (!parsed.success) {
        return null;
      }
      const permissions = toPendingInteractionGrantablePermissionProfile(parsed.data.permissions);
      return {
        requestId: request.id,
        method: request.method,
        providerThreadId: parsed.data.threadId,
...
export function buildCodexInteractiveResponse(
  ...
  switch (args.request.payload.subject.kind) {
    ...
    case "permission_grant": {
      ...
      const response: PermissionsRequestApprovalResponse = {
        permissions: toCodexGrantedPermissionProfile(requireGrantedPermissions(args.resolution)),
