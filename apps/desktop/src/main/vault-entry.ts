...
export const resolveVaultEntry = (args: ResolveVaultEntryArgs): VaultEntryVerdict => {
  ...
  try {
    ...
    entryReal = args.realpath(path.join(args.vaultDir, ...parsed.path.split("/")));
  } catch {
  ...
  return { absPath: entryReal, ok: true };
};
