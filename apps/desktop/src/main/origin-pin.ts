...
const comparableOrigin = (value: string): string | null => {
  let url: URL;
  try {
    url = new URL(value);
  } catch {
...
export const isSameOriginNavigation = (targetUrl: string, appOrigin: string): boolean => {
...
export const classifyNavigation = (targetUrl: string, appOrigin: string): NavigationVerdict => {
...
export const classifyWindowOpen = (url: string): WindowOpenVerdict =>
  isHttpUrl(url) ? "deny-and-open-external" : "deny";
...
export const decideExternalOpen = (args: ExternalOpenArgs): ExternalOpenDecision => {
...
export const appWindowWebPreferences = (preloadPath: string, partition: string) =>
  ({
