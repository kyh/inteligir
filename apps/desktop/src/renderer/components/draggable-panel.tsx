...
export function DraggablePanel({
  ...
  const handleResizeStart = useCallback(
    (handle: ResizeHandle, e: React.PointerEvent) => {
      ...
      const handleMove = (moveEvent: PointerEvent) => {
        if (!resizeStartRef.current || moveEvent.pointerId !== e.pointerId) return;
  ...
  const handleDragStart = useCallback(
    (e: React.PointerEvent) => {
      ...
      const handleMove = (moveEvent: PointerEvent) => {
        if (!dragStartRef.current || moveEvent.pointerId !== e.pointerId) return;

        let newLeft = dragStartRef.current.left + (moveEvent.clientX - dragStartRef.current.x);
        let newTop = dragStartRef.current.top + (moveEvent.clientY - dragStartRef.current.y);

        newLeft = Math.max(0, Math.min(newLeft, window.innerWidth - size.width));
        newTop = Math.max(0, Math.min(newTop, window.innerHeight - size.height));
  ...
  function ResizeHandleEl({ handle }: { handle: ResizeHandle }) {
    ...
    return (
      <div
        ...
        style={{
          width: isCorner ? 12 : handle === "n" || handle === "s" ? "100%" : 8,
          height: isCorner ? 12 : handle === "e" || handle === "w" ? "100%" : 8,
  ...
  const panel = (
    ...
    >
      ...
      <ResizeHandleEl handle="nw" />
      <ResizeHandleEl handle="ne" />
      <ResizeHandleEl handle="sw" />
      <ResizeHandleEl handle="se" />
      <ResizeHandleEl handle="n" />
      <ResizeHandleEl handle="s" />
      <ResizeHandleEl handle="e" />
      <ResizeHandleEl handle="w" />
    </motion.div>
  );
