// A surface reads through the one oRPC client it ships with, so a test answers the wire
// (`{ json }` in, `{ json }` out) per procedure. An answer that throws or rejects is the server
...
type Answer = (input: Json | undefined, signal: AbortSignal | undefined) => Json | Promise<Json>;
...
export const stubRpc = (answers: Partial<Record<string, Answer>>): void => {
  vi.stubGlobal("fetch", async (input: string | URL | Request, init?: RequestInit) => {
    ...
    let output: Json;
    try {
      output = await answer(json, init?.signal ?? undefined);
    } catch {
