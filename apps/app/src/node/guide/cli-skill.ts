...
// skill verbatim. DOC-SYNC: this manual must name every CLI leaf command —
// apps/cli/src/__tests__/guide-covers-commands.test.ts walks the real command
// tree against this file's bytes and fails on any command it does not
// mention, so adding or renaming a CLI command means updating this text in
// the same change (the discipline bb records in docs/cli-guide-and-skill.md).
...
- \`INTELIGIR_SERVER_URL\` — when set (it is set inside agent shells), the CLI
  dials it directly. This is the base URL, e.g. \`http://127.0.0.1:21847\`.
- Otherwise the CLI derives the local instance the same way the app does
  (per-checkout dev port, then the installed default) and probes for it.
...
- \`inteligir vault write <path> [--content <text>]\` — write a file; without
  \`--content\` the content is read from stdin. Parent folders are created.
...
- \`inteligir thread new [--doc <path> --anchor <a>] <prompt>\` — create a
  thread (optionally bound to a doc anchor) and send the first turn.
...
- \`inteligir thread wait <id>\` — block until the thread settles. Exit code
  0 = idle, 1 = settled in error, 2 = timeout (\`--timeout <s>\`,
  \`--poll-interval <ms>\`).
...
- \`inteligir interactions answer <id> <resolution>\` — answer one;
  resolutions are \`allow_once\`, \`allow_for_session\`, or \`deny\`.
