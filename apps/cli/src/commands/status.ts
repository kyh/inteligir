...
// context (which server was dialed, which thread the shell runs in).
...
export function registerStatusCommand(program: Command, deps: CliDeps): void {
  program
    .command("status")
    .description("Server version, data dir, agent runtime state, thread context")
    .option("--json", "Print machine-readable JSON output")
    .action(async (opts: JsonOutputOptions) => {
      const baseUrl = await deps.resolveServer();
      const api = await apiFor(deps);
      const response = await api.system.status.$get();
      const body = await response.json();
      const threadId = contextThreadId(deps.env) ?? null;
      if (outputJson(opts, { serverUrl: baseUrl, contextThreadId: threadId, ...body })) {
        return;
      }
      console.log(`inteligir ${body.version} — ${baseUrl}`);
