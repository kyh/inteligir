...
import type { VaultServiceErrorCode } from "./vault-service";
...
const vaultWireClass = (cause: unknown): VaultWireClass | null => {
  if (cause instanceof VaultPathError) {
    return VAULT_REFUSALS.invalid_path;
  }
  if (cause instanceof VaultServiceError) {
    return VAULT_REFUSALS[cause.code];
  }
  return null;
};

export const vaultWireError = (cause: unknown): ORPCError<string, unknown> | null => {
  const wireClass = vaultWireClass(cause);
  if (wireClass === null || !(cause instanceof Error)) {
    return null;
  }
  return new ORPCError(wireClass, { message: cause.message });
};

export const vaultRefusalStatus = (cause: unknown): number | null => {
  const wireClass = vaultWireClass(cause);
