...
describe("ensureDevDataDirOwnership", () => {
  it("records the checkout on first boot and accepts it thereafter", () => {
    ...
    const dataDir = path.join(root, "instance");
    ensureDevDataDirOwnership(dataDir, "/checkout/a");
    expect(readFileSync(path.join(dataDir, "checkout-path"), "utf-8").trim()).toBe("/checkout/a");
    expect(() => {
      ensureDevDataDirOwnership(dataDir, "/checkout/a");
    }).not.toThrow();
  });
  ...
  it("refuses a data dir recorded for a different checkout", () => {
    ...
    ensureDevDataDirOwnership(dataDir, "/checkout/a");
    expect(() => {
      ensureDevDataDirOwnership(dataDir, "/checkout/b");
    }).toThrow(/belongs to checkout "\/checkout\/a".*INTELIGIR_DATA_DIR/su);
  });
});
