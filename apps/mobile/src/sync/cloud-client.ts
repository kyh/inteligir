// The typed face over the cloud Worker — the RN twin of
// apps/app/src/node/cloud/cloud-client.ts, and the same ONE thing it exists to
// get right: a refusal is a VALUE, never a thrown string.
//
// The contract ships a closed error enum precisely so the client can switch on
// it, and the sync loop's behaviour hangs off that switch: `unauthorized` stops
// the runtime, `sync-conflict`/`sync-out-of-order` mean this device's own outbox
// is wrong (and name the position), everything else is worth one more attempt.
//
// Three failure kinds, not one, because the callers differ: a REFUSED call
// carries the server's judgement, an UNREACHABLE one carries no judgement at all
// (offline, a captive portal) and must not read as a verdict on the credential,
// and a MALFORMED answer is a body this build cannot read — what a proxy that
// intercepted the request looks like.
 ...
 *  string URL, and the narrow shape is what a test double implements. */
...
/** One sentence for a status line — the UI renders this, so it says what
...
/** How long any one cloud call may take. Every call is made from a single-flight
 *  pass, so one black-holed request stalls the whole loop, not one request. */
...
export interface CloudEndpoint {
  ...
  baseUrl: string;
  /** Injected so the suite can drive the runtime without a network. */
  fetch?: CloudFetch;
  /** Cancels in-flight calls — the runtime's own, aborted on dispose. Composed
   *  with the per-request timeout, never replacing it. */
  signal?: AbortSignal;
}
 ...
 * short-lived, rate-limited and PKCE-bound on the far side.
 */
...
export function createCloudClient(args: CreateCloudClientArgs): CloudClient {
  ...
  /** `json` present means POST with a body; absent means GET. Headers are built
   *  here, so the bearer can never be dropped by a caller's own header list. */
