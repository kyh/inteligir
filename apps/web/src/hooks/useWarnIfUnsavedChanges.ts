...
export const useWarnIfUnsavedChanges = (options: {
  ...
  // Intercept router navigation - valid Effect (external Next.js router)
  useEffect(() => {
    if (!options?.router) return;

    const originalPush = router.push;

    // eslint-disable-next-line react-hooks/immutability -- valid: patching Next.js router for navigation interception
