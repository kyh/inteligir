...
const InsertLinkCommand = ({ initialUrl }: { initialUrl: string }) => {
  ...
  // Sync search results from API or filter local data - valid Effect (external API data)
  useEffect(() => {
    if (session) {
      if (!searchData?.documents) return;

      // eslint-disable-next-line react-hooks/set-state-in-effect -- valid: syncing from external API
...
const EditLinkCommand = ({
  ...
  // Sync text from editor node - valid Effect (external editor state that user can modify)
  useEffect(() => {
    if (editingLinkEntry) {
