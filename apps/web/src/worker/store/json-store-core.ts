// ---------------------------------------------------------------------------
// The JsonStore engine: TypeBox validation on read AND write, version drift as
// quarantine-and-reset, the read cache, and the close() kill switch. Every
// byte-level operation goes through an injected StoreAdapter, so the engine
// itself never names a storage medium — do-store-adapter.ts binds it to a
// Durable Object's synchronous KV. The store never parses `filePath` either:
// it is a KV key here, and quarantine only ever suffixes it.
// ---------------------------------------------------------------------------
 ...
 *
 * SYNCHRONOUS IS A CONTRACT, not a convenience. The capture inbox's
 * exactly-once compare-and-swap is correct only because a read-modify-write
 * completes in one JS turn, with no await for another caller to interleave on.
 * Never widen these to promises — a SQLite-backed Durable Object's KV exposes
...
export class JsonStoreCore<T> {
  ...
  write(data: T): void {
    ...
    this.adapter.write(this.filePath, JSON.stringify(this.encode(this.cache), null, 2));
  }
