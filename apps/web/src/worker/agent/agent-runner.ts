...
import { agentToolManifest, executeAgentTool } from "./agent-tools";
...
const BOOT_ID_KEY = "agent/boot-id";
...
export class AgentRunner {
  ...
  /**
   * Whether a turn is in flight, as far as this object knows.
   *
   ...
   * to believe otherwise until the next report arrives.
   */
  private busy = false;
  ...
  async send(command: TextChatMessage): Promise<void> {
    if (command.type === "interrupt") {
      const outcome = await this.deps.sandbox().interrupt();
    ...
    if (provider === null) {
      throw new Error("No AI provider is selected. Choose one in Settings → AI.");
    }
    if (!this.deps.credentials.connected(provider.id)) {
      throw new Error(`${provider.label} is not connected. Connect it in Settings → AI.`);
    }
    ...
    // not go.
    const port = this.deps.sandbox();
    const seed = await this.ensureContainer(port, provider);
  ...
  /**
   * One report from the container.
   *
   * The caller has already proven the bearer; this is where a report BECOMES
   * something — a transcript entry, a broadcast, a vault write, a tool result.
   * The scripted sandbox calls it directly, so this is the same path in both
   * runtimes.
   */
  async report(report: AgentReport): Promise<AgentReportReply> {
    switch (report.kind) {
      case "events":
        for (const raw of report.events) this.applyEvent(raw);
        return { kind: "ack" };
      case "tool": {
        const result = await executeAgentTool(
          {
            ...
            sessionId: this.deps.chat.activeSessionId(),
      ...
      case "vault":
        return this.applyVaultOps(report.ops);
      case "turn_end":
        this.setBusy(false);
   ...
   * boot, then mint a provider access token. The egress interceptor's only way
  ...
  /** Whether a report bearer is this object's, for the report route's own
   * check. Same two conditions as `mintProviderAccessToken`. */
  async acceptsReportToken(identity: string): Promise<boolean> {
    const claims = await verifyScopedToken(
      this.deps.env.BETTER_AUTH_SECRET,
      "report",
      identity,
    ...
    return claims !== null && claims.userId === this.deps.userId && claims.ref === this.bootId();
  }

  // ---- container lifecycle --------------------------------------------------

  /**
   * Bring the container to a state that can run a turn, and say what the turn
   * must seed it with.
   *
  ...
  private async ensureContainer(
    ...
    const warm = state.phase === "ready" && state.bootId === this.bootId();
    ...
    if (!warm) {
      ...
      this.deps.kv.put(BOOT_ID_KEY, bootId);
    }
  ...
  private applyParsedEvent(event: AppAgentEvent): void {
   ...
   */
  private async applyVaultOps(ops: readonly VaultOp[]): Promise<AgentReportReply> {
    const sessionId = this.deps.chat.activeSessionId();
    ...
    for (const op of ops) {
      ...
      if (!(await this.deps.snapshots.capture(sessionId, op.path))) {
  ...
  private bootId(): string {
    const stored = this.deps.kv.get(BOOT_ID_KEY);
    return typeof stored === "string" ? stored : "";
  }

  /**
   * Where the container posts its reports, or `null` when this deployment has
   * nowhere to name.
   *
