...
// image. Five verbs is what the agent actually needs, and five verbs is what an
// in-memory fake can implement completely (./fake-sandbox), so the tests drive
// the SAME runner, tool executor and transcript the real binding drives.
//
...
// it reached a socket. `probe` is the one that must never fail at all: it is how
// the runner learns the container was wiped.
//
...
export type SandboxState =
  ...
  | {
      ...
      /** The transcript sequence the live pi session was seeded through. */
...
export type SandboxBoot = {
  /** The boot identity the container echoes back from `probe`. */
  readonly bootId: string;
  /** Absolute URL the container posts every report to (./agent-route). */
  readonly reportUrl: string;
  /** The bearer the container presents on every report (./callback-token). */
...
export type SandboxTurn = {
  ...
  /** Prior turns to seed with — empty when the live session is already seeded
   * past the transcript's head. */
  readonly seed: readonly SandboxSeedTurn[];
  /** The transcript sequence `seed` carries the container through. */
  readonly seededThrough: number;
};
