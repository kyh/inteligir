...
- **On the command line:** `inteligir open` opens another signed-in browser tab; `inteligir action list` pages with `--limit` and `--cursor`, filters with `--doc` and `--running`, and leaves archived actions out unless you pass `--archived`; `inteligir action wait` names an approval it is waiting on, and `--until-input` exits 4 when one arrives; `inteligir action changes` lists the notes each of an action's turns changed; `inteligir agents list` shows which agents are installed and signed in.
