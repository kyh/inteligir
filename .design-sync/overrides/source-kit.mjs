...
// forked from design-sync lib/source-kit.mjs — exclude react-three-fiber files
// from the synth entry (they bundle a 2nd React reconciler that imports
// `scheduler` directly, which the DS bundle stubs with a throwing shim →
// [SCHEDULER_MISSING] blows up the whole IIFE). GeometricOrb is the only such
// file; dropping it from the bundle also removes the WebGL card and ~3MB of
// three.js. Imports repoint at the staged lib; bare deps (ts-morph) resolve via
// the .design-sync/node_modules -> ../.ds-sync/node_modules symlink (prepare.mjs).
...
// A src file that imports @react-three/fiber or react-reconciler brings its own
// React reconciler (+ scheduler). Such files can't live in a bundle that
// externalizes react-dom and stubs scheduler — exclude them from the entry.
const SECOND_RECONCILER_RX = /(?:^|[^.\w])(?:@react-three\/fiber|react-reconciler)(?:['"/]|$)/m;
function importsSecondReconciler(p) {
  try {
    return SECOND_RECONCILER_RX.test(readText(p));
  } catch {
    return false;
  }
}
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const comps = srcFiles.filter(
      (p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p) && !importsSecondReconciler(p),
    );
